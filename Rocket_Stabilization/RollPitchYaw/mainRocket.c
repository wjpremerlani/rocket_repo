...
void send_debug_line(void)
{
	...
	{
		firstline = 0 ;
		sprintf( debug_buffer , "vertX, vertY, vertZ, accX, accY, accZ, gyroX, gyroY, gyroZ, in1 , in2 , in3 , in4\r\n" ) ;
	}
	else
	{
		sprintf(debug_buffer, "%i, %i, %i, %i, %i, %i, %i, %i, %i, %i, %i, %i, %i\r\n",  
			...
			( udb_xrate.value)/2 - ( udb_xrate.offset ) / 2 ,
			( udb_yrate.value)/2 - ( udb_yrate.offset ) / 2 ,
			( udb_zrate.value)/2 - ( udb_zrate.offset ) / 2 ,
			udb_pwIn[1] ,
			udb_pwIn[2] ,
			udb_pwIn[3] ,
			udb_pwIn[4] );
